...
The proposed approach here is to calculate the average color rendered by all all possible bit patterns that one can generate on a small patch or block of pixels, which will reveal interesting features in both layers.  Next is to determine a usable block size.  Since the color encoding layer has a size of 2x1 bits (wxh), in a full resolution of 280x192, rendering 4 colors + black and white brings us already to a color resolution of 140x192.
