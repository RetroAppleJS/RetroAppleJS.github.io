...
DOCS_TOOLS = "<h2 id=\"tools-explained\">TOOLS Explained</h2><hr>"
...
+"<p>In this project, we aim to have all the documentation available in markdown format, because it is easy to access, read and edit in GitHub.  As the IDE is a client-side application, security restrictions prevent these markdown files to be included into the main application.  Docs_updater.html was designed to read all the markdown documentation available online in this project, and compile a JavaScript include file that would provide a perfect copy of all the available documentation inside the IDE.</p>"
+"<pre><code>┌──────────┐          ┌─────────────┐        ┌──────────────────┐<br>"
+"│Readme.md │          │ Showdown.js │        │ COM_markdown.css │<br>"
+"└────┬─────┘          └──────┬──────┘        └──────┬───────────┘<br>"
+"     │XmlHttpRequest()       │                      |<br>"
+"┌────┴────────┐     ┌────────┴───────┐       ┌──────┴───────────┐     ┌────────────────────┐<br>"
+"│Filter all   │     │Convert Markdown│       | Preview/test HTML|     | Download JS file   |<br>"
+"│Markdown docs├─────┤to HTML + append├───┐───┤ generated by     ├─────┤ _GENERATED_DOCS.js | <br>"
+"│in /docs dir │  ▲  │to JS queue     │   │   | JavaScript file  |     |(to upload in /docs)|<br>"
+"└─────────────┘  │  └────────────────┘   │   └──────────────────┘     └────────────────────┘<br>"
+"                 │    loop until done    │<br>"
+"                 └───────────────────────┘</code></pre>"
+"<p>⚠️ DOCS_updater.html does not get markdown content updates instantateously, only 2-10 min after your last markdown edits, the updater will get the latest version. </p>"
...
DOCS_EMULATOR = "<h2 id=\"emulator-instructions\">EMULATOR Instructions</h2><hr>"
