...
async function printPaginated(rl, text) {
  ...
  for (let i = 0; i < lines.length; i += PAGE_LINES) {
    ...
    if (i + PAGE_LINES < lines.length) {
      await ask(
        rl,
        COLOR_GPT + "--- Press ENTER to continue ---" + COLOR_RESET
      );
    }
  }
}
...
async function runConversation(models) {
...
Commands:
  ...
  /show_models   Show all available models
  /quit          Quit
`);
    ...
    if (lower === "/show_models") {
      if (!models || !models.length || models[0] === "(unavailable)") {
        console.log("Model list unavailable.");
      } else {
        const text =
          "Available models:\n" +
          models.map((m) => "  - " + m).join("\n");
        await printPaginated(rl, text);
      }
      await ask(rl, "Press ENTER to continue...");
      continue;
    }
...
(async () => {
  if (!API_KEY) {
    console.error("ERROR: Set your API key: export OPENAI_API_KEY=xxxxx");
    process.exit(1);
  }

  const models = await getAvailableModels();
  await runConversation(models);
})();
