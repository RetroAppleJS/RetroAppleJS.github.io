...
function rx(re,replacement,label)
{
    if(!re.test(src)) throw new Error(label+': pattern not found');
    src=src.replace(re,replacement);
}

rx(/var deviceSurfaceMapState = \{"unit":null,"hash":null\};/,
   'var deviceSurfaceMapState = {"unit":null,"hash":null,"page":0};','surface state');
rx(/var canMap=deviceCode==="UNIDISK" && typeof\(device\.getSurfaceMapGeometry\)==="function" && !!state\.mediaLoaded;/,
   'var canMap=(deviceCode==="UNIDISK" || deviceCode==="HD20") && typeof(device.getSurfaceMapGeometry)==="function" && !!state.mediaLoaded;','sync capability');
rx(/if\(!device \|\| device\.id\?\.DCODE!=="UNIDISK" \|\| typeof\(device\.getSurfaceMapGeometry\)!=="function"\) return null;/,
   'if(!device || (device.id?.DCODE!=="UNIDISK" && device.id?.DCODE!=="HD20") || typeof(device.getSurfaceMapGeometry)!==="function") return null;','surface target');
...
rx(/    this\.deviceToolSurfaceMapHTML = function\(unit,expectedHash\)\n    \{/,
   helpers+'    this.deviceToolSurfaceMapHTML = function(unit,expectedHash)\n    {','helper insertion');
rx(/        if\(!device\)\n            return header\+"<div class=\\"liron-surface-status\\">Device instance is no longer attached\.<\/div>";\n\n        var instance=deviceToolInstanceHex\(device\) \|\| "\?\?\?\?";/,
   '        if(!device)\n            return header+"<div class=\\"liron-surface-status\\">Device instance is no longer attached.</div>";\n\n        if(device.id?.DCODE==="HD20")\n            return deviceToolHD20SurfaceMapHTML(device,header);\n\n        var instance=deviceToolInstanceHex(device) || "????";','HD20 render dispatch');
rx(/        if\(!state\.mediaLoaded\) return false;\n\n        var image=typeof\(device\.getImage\)==="function" \? device\.getImage\(\) : null;/,
   '        if(!state.mediaLoaded) return false;\n\n        if(device.id?.DCODE==="HD20")\n            return deviceToolHD20SurfaceMapUpdate(device,text);\n\n        var image=typeof(device.getImage)==="function" ? device.getImage() : null;','HD20 update dispatch');
rx(/        popup\.style\.width=Math\.min\(326,available\)\+"px";/,
   '        var surfaceDevice=deviceToolSurfaceTarget(deviceSurfaceMapState.unit,deviceSurfaceMapState.hash);\n        var desiredWidth=surfaceDevice && surfaceDevice.id?.DCODE==="HD20" ? 414 : 326;\n        popup.style.width=Math.min(desiredWidth,available)+"px";','popup width');
rx(/            var isUniDisk=deviceCode==="UNIDISK" && typeof\(device\.getSurfaceMapGeometry\)==="function";/,
   '            var supportsSurfaceMap=(deviceCode==="UNIDISK" || deviceCode==="HD20") && typeof(device.getSurfaceMapGeometry)==="function";','toolbox surface variable');
rx(/,"capabilityActions":isUniDisk \? \[\{"id":controlID\+"_surface"/,
   ',"capabilityActions":supportsSurfaceMap ? [{"id":controlID+"_surface"','toolbox surface action');
rx(/        if\(opening\)\n        \{\n            deviceSurfaceMapState\.unit=unit;\n            deviceSurfaceMapState\.hash=expectedHash;/,
   '        if(opening)\n        {\n            var targetChanged=deviceSurfaceMapState.unit!==unit || deviceSurfaceMapState.hash!==expectedHash;\n            deviceSurfaceMapState.unit=unit;\n            deviceSurfaceMapState.hash=expectedHash;\n            if(targetChanged) deviceSurfaceMapState.page=0;','toggle page reset');
rx(/        deviceSurfaceMapState\.unit=unit;\n        deviceSurfaceMapState\.hash=expectedHash;\n        if\(!liron\.deviceToolSurfaceMapRefresh\(\)\) return false;/,
   '        var targetChanged=deviceSurfaceMapState.unit!==unit || deviceSurfaceMapState.hash!==expectedHash;\n        deviceSurfaceMapState.unit=unit;\n        deviceSurfaceMapState.hash=expectedHash;\n        if(targetChanged) deviceSurfaceMapState.page=0;\n        if(!liron.deviceToolSurfaceMapRefresh()) return false;','open page reset');
