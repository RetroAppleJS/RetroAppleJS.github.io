...
*/
    ...
    this.dNd = {enable:false,motor:"OFF",status:"",detune:0,rept:0,last:{}};  // disk Noise data
    this.AUD_buffer={};
   ...
   this.ROM = new Uint8Array([ // P5A
    ...
    this.traceNibKind = function(bytes)
    {
        if (bytes.length >= 3)
        {
            if (bytes[0] == 0xd5 && bytes[1] == 0xaa && bytes[2] == 0x96)
                return "ADDR_FIELD_PROLOGUE";

            if (bytes[0] == 0xd5 && bytes[1] == 0xaa && bytes[2] == 0xad)
                return "DATA_FIELD_PROLOGUE";

            if (bytes[0] == 0xde && bytes[1] == 0xaa)
                return "FIELD_EPILOGUE";
        }
    ...
    this.traceLog = function(eventName, data)
    {
        ...
        if (this.trace.events.length > this.trace.maxEvents)
            this.trace.events.shift();
...
//id='"+popup_id+"' hidden='' class='appbox com_popup_frame' style='position:absolute;left:850px;width:300px;height:250px;text-align:left;padding:0px;margin:0px'
    ...
    this.getSoftwareCatRows = function(arg)
    {
        ...
        function GH_listDir(elid, owner, repo, path, ref, callback)
        {
            ref = ref || "main";
            path = path || "";
        ...
        GH_listDir(
            ...
            "disks",
            "main",
            function(err, list, elid)             // CALLBACK FUNCTION
            {
                if(err)
                {
                    console.warn("GitHub directory listing failed", err);
                    return;
                }
    ...
    this.surfaceMap_grid_html = function(drv)
    {
        ...
        for(var track=0; track<35; track++)
        {
            ...
            {
                s.push(
                    "<td id=\"surfaceMap_D"+d+"_"+track+"_"+sector+"\""
    ...
    this.surfaceMap_update = function(popup_id)
    {
        ...
        for(var drv=0; drv<2; drv++)
        {
            ...
            try
            {
                dskBytes = this.convertNib2Dsk(this.state.diskData[drv]);
            }
            catch(e)
            {
            ...
            if(status)
                status.innerHTML = dskBytes.length+" bytes";

            for(var track=0; track<35; track++)
            {
                for(var sector=0; sector<16; sector++)
                {
                    var offs = (track << 12) + (sector << 8);
                    var nz = 0;
                    var sum = 0;

                    for(var i=0; i<256; i++)
                    {
                        var b = dskBytes[offs+i] & 0xff;
                        sum += b;
                        if(b != 0) nz++;
                    }
