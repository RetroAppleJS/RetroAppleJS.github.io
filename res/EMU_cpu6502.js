...
function Cpu6502(hwobj)
{
    ...
    var BOOTlog_adr = new Uint16Array(BOOTsiz);
    var BOOTlog_cpu = new BigUint64Array(BOOTsiz);

    const BOOTlog_exclude = new Set([
        //0xFCAA, 0xFCAC, 0xFCAE, 0xFCAF, 0xFCB1, 0xFCA9
    ]);
...
//   ██████ ██       ██████       ██████  ██████  ██   ██ ███████ 
    ...
    this.cycle = function()
    {
        ...
        const exclude = [0xFCAA,0xFCAC,0xFCAE,0xFCAF,0xFCB1,0xFCA9];

        var boot_pc = instr_pc & 0xffff;

        if (bDebug_boot && BOOTcnt < BOOTsiz && !BOOTlog_exclude.has(boot_pc))
        {
            BOOTlog_adr[BOOTcnt] = boot_pc;
            BOOTlog_cpu[BOOTcnt] = packBootState(opcode, operand);
            BOOTcnt++;

            if (BOOTcnt == BOOTsiz)
            {
                alert("cpu log ended");
                console.log("BOOTlog = " + this.getBootLogBase64());
            }
        }
...
//  ██       ██████  ██   ████  ██████    ██    ██  ██████  ██   ████ ███████ 
    ...
    this.getBootLogBase64 = function()
    {
        ...
        for (var i = 0, o = 0; i < BOOTcnt; i++, o += BOOT_RECORD_BYTES)
        {
            ...
            var cpu = BOOTlog_cpu[i];   // BigInt

            bytes[o    ] = adr & 0xff;
            bytes[o + 1] = (adr >> 8) & 0xff;

            bytes[o + 2] = Number( cpu        & 0xffn);  // opcode
            bytes[o + 3] = Number((cpu >> 8n)  & 0xffn);  // operand low
            bytes[o + 4] = Number((cpu >> 16n) & 0xffn);  // operand high
            bytes[o + 5] = Number((cpu >> 24n) & 0xffn);  // A
            bytes[o + 6] = Number((cpu >> 32n) & 0xffn);  // X
            bytes[o + 7] = Number((cpu >> 40n) & 0xffn);  // Y
            bytes[o + 8] = Number((cpu >> 48n) & 0xffn);  // P
            bytes[o + 9] = Number((cpu >> 56n) & 0xffn);  // SP
        }
