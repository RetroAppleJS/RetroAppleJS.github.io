...
function RamCard()
{
    ...
    this.MEM_status_id = "MS16K_softswitch_status";

    this.MEM_status_text = function()
    {
        var sw = softswitch[this.state.softswitch_pos] || {};
        var on_icon = (sw.ON==1)
            ? '<i class="fa fa-play-circle" title="MS16K ON"></i>'
            : '<i class="fa fa-pause-circle" title="MS16K OFF"></i>';
        var we_icon = (sw.WE==1)
            ? '<i class="fa fa-lock-open"  title="MS16K WRITE ENABLE"></i>'
            : '<i class="fa fa-lock" title="MS16K WRITE PROTECTED"></i>';
        var bank = (sw.BANK==1) ? '<span title="MS16K RAM BANK B">B</span>' : '<span title="MS16K RAM BANK A">A</span>';

        return "MS16K " + on_icon + " " + we_icon + " " + bank;
    };
    ...
    this.update_MEM_status = function()
    {
        ...
        var el = document.getElementById(this.MEM_status_id);
        if(!el) return false;
    ...
    this.MEM_monitoring = function()
    {
        ...
        if(bDebug_mon) console.log(JSON.stringify( "MEM_monitoring_MS16K -> bankA:"+JSON.stringify(rc.mem_mon.bankA)+" bankB:"+JSON.stringify(rc.mem_mon.bankB)+" ramcard:"+JSON.stringify(rc.mem_mon.ramcard)) );
