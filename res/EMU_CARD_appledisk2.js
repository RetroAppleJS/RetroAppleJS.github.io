...
function AppleDisk2()
{
    ...
    // One bounded buffer for bDebug, bDebug_N, semantic trace and soft switches.
    var ssTrace = [];
    var ssTraceSeq = 0;
    ...
    function diskLogEnabled()
    {
        return bDebug || bDebug_N || bDebug_SS;
    }
    ...
    function diskLogAppend(entry,enabled,preserveDataRun)
    {
        if(!enabled) return false;

        // Keep an active Q6L aggregate in chronological order with other events.
        if(!preserveDataRun && ssDataRun)
            ssFlushDataRun("interleaved-log");
        ...
        if(ssTrace.length>=SS_TRACE_LIMIT)
        {
            ...
            if(!ssTraceLimitWarned)
            {
                ...
                console.warn(
                    "AppleDisk2: disk log limit reached; additional records are dropped"
                );
            }
        ...
        if(entry.slot===undefined) entry.slot = ssSlotLabel();
        if(entry.seq===undefined) entry.seq = ++ssTraceSeq;

        ssTrace.push(entry);
        return true;
    }

    function diskLogEvent(type,data,enabled)
    {
        var entry = {"type":type,"slot":ssSlotLabel()};
        data = data || {};
        ...
        return diskLogAppend(entry,enabled,false);
    }
    ...
    function ssFlushDataRun(reason)
    {
        ...
        diskLogAppend(ssDataRun,bDebug_SS,true);
        ssDataRun = null;
    }
    ...
    this.traceSoftSwitch = function(op,addr,d8,ctx,before,after,result,note)
    {
        ...
        diskLogAppend(entry,bDebug_SS,true);
    };
    ...
    this.downloadDiskLog = function()
    {
        if(
            !diskLogEnabled() ||
            typeof(oCOM)!="object" ||
            typeof(oCOM.Download)!="function"
        )
            return false;

        if(this.trace && this.trace.enabled)
            this.traceFlush("download");

        ssFlushDataRun("download");

        var slot = ssSlotLabel();
        var json = JSON.stringify({
             "enabled":diskLogEnabled()
            ,"slot":slot
            ,"records":ssTrace.slice()
            ,"dropped":ssTraceDropped
        }).replace( /\},\{/g , "},\n{" );

        oCOM.Download(
             "DiskII_"+slot+"_disklog.json"
            ,new TextEncoder("utf-8").encode(json)
        );
    ...
    this.flushTrackStats = function(deviceN, reason)
    {
        ...
        var read = stats.read || 0;
        var write = stats.write || 0;

        // Avoid noisy empty messages.
        if (read == 0 && write == 0)
            return;

        diskLogEvent("TRACK_STATS",{
             "source":"bDebug"
            ,"drive":ssSlotLabel()+".D"+(deviceN+1)
            ,"drv":deviceN
            ,"track":track
            ,"read":read
            ,"write":write
        ...
        // Preserve the existing console diagnostic.
        console.log(
    ...
    */
    ...
    this.downloadDisk = function(deviceID)
    {
        ...
        var nibBytes = state.diskData[deviceN];
        if(nibBytes==null) return;

        var fileName = this.getDiskName(deviceID);
        var match = fileName.toLowerCase().match(/\.([a-z0-9]+)$/);
        var ext = match ? match[1] : "dsk";
        var bytes;

        switch(ext)
        {
            case "nib":
                // The live emulator representation already is NIB.
                bytes = Array.from(nibBytes);
                break;

            case "po":
                // Restore ProDOS sector ordering.
                bytes = this.convertNib2Po(nibBytes);
                break;

            case "dsk":
            case "do":
                // Restore DOS 3.3 sector ordering.
                bytes = this.convertNib2Dsk(nibBytes);
                break;

            case "woz":
                // Do not create a DSK image carrying a false .woz extension.
                console.warn(
                    "Cannot download " + fileName +
                    " as WOZ: WOZ conversion is not implemented."
                );
                return;

            default:
                // Preserve the historical behaviour for unnamed/unknown images.
                bytes = this.convertNib2Dsk(nibBytes);
                break;
        }

        oCOM.Download(fileName, bytes);
    }
...
this.detectDiskImageType = function(imageBytes, filepath)
{
    ...
    function dN_log(event,message,data)
    {
        if(!bDebug_N) return;

        // Preserve the existing console diagnostics.
        console.log(message);

        data = data || {};
        data.source = "bDebug_N";
        data.event = event;
        diskLogEvent("DISK_NOISE",data,true);
    }
    ...
    this.dN_spindown = function(t)
    {
        dN_log(
             "TIMER_EXPIRED"
            ,"TIMER EXP ("+t.dNd.motor+") - "+t.dNd.state
            ,{"motor":t.dNd.motor,"state":t.dNd.state}
        );
        ...
        if(t.dNd.state=="MOTOR_OFF")
        {
            ...
            dN_log(
                 "SPINDOWN"
                ,"dN_spindown: dNd.motor="+t.dNd.motor
                ,{"motor":t.dNd.motor,"state":t.dNd.state}
            );
   ...
   const ROM = new Uint8Array([ // P5A
    ...
    this.traceLog = function(eventName, data)
    {
        ...
        diskLogEvent("TRACE",e,true);

        // Preserve the existing semantic trace console output.
        if (this.trace.echo && typeof console != "undefined")
            console.log(this.traceFormat(e));
    }
...
""+
""+
""+
""+
""+
""+
""+
""+
""+
""+
""+
""+
""+
""+
""+
""+
""+
""+
""+
""+
""+
""+
""
        },
        "disks/BLANK_DOS.DSK":
        {
encoding:"zlib-base64",
data:""+
""+
""+
""+
""+
""+
""+
""+
""+
""
        },
...
""
    ...
    this.getFile = function(arg)
    {
        try
        {
            ...
            function mountDiskBytes(bytes, source)
            {
                ...
                var info = disk2.detectDiskImageType(bytes, arg.path);
                var nibBytes;
    ...
    {
        return oCOM.CATALOG.isOffline();
    }

    this.getOfflineDir = function(url)
    {
        return oCOM.CATALOG.offlineDirectory(
            GitHubDirOffline,
            url
        );
    }
    ...
    {
        var bytes = oCOM.CATALOG.offlineData(
            OfflineDisks,
            arg,
            full_path,
            "bytes"
    ...
    this.setDriveCatalogFile = function(drv, fileName)
    {
        try
        {
            ...
            // Keep the mounted-image metadata in sync with the catalog UI.
            // downloadDisk() uses this per-drive name.
            this.setDiskName(drv, fileName);
