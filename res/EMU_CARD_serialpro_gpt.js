...
(function(global)
{
    ...
    const KRM_MARK = 0x01;
    const KRM_EOL = 0x0D;
    const KRM_MAX_DATA = 80;       // inbound parser limit / KFS1 maximum
    const KRM_TX_DATA = 32;        // conservative outbound D payload for 1 MHz + DOS 3.3
    const KRM_RETRIES = 5;
    const KRM_TIMEOUT_DEFAULT_MS = 10000;
    const KRM_TIMEOUT_MS_BY_TYPE = Object.freeze({
         S:5000
        ,F:20000
        ,D:15000
        ,Z:20000
    ...
    {
        type = String(type || "");
        return KRM_TIMEOUT_MS_BY_TYPE[type] || KRM_TIMEOUT_DEFAULT_MS;
    }

    function serialGPTKermitWait(state,timeoutMs)
    {
        var k = serialGPTKermitState(state);
        timeoutMs = Number(timeoutMs);
        if(!Number.isFinite(timeoutMs) || timeoutMs<=0)
            timeoutMs = KRM_TIMEOUT_DEFAULT_MS;
        ...
        return new Promise(function(resolve,reject)
        {
            ...
            var timer = setTimeout(function()
            {
                ...
                reject(new Error("Kermit timeout after "+timeoutMs+" ms"));
            },timeoutMs);
    ...
    async function serialGPTKermitSendAndAck(card,seq,type,data)
    {
        ...
        var timeoutMs = serialGPTKermitTimeoutForType(type);
        for(var attempt=0;attempt<KRM_RETRIES;attempt++)
        {
            serialGPTStatus(card,
                "KERMIT TX "+type
                +" seq="+seq
                +" bytes="+data.length
                +" attempt="+(attempt+1)+"/"+KRM_RETRIES
                +" timeout="+timeoutMs+"ms");            
            serialGPTKermitTx(card,packet);
            try
            {
                var reply = await serialGPTKermitWait(state,timeoutMs);

                if(reply.seq!==seq)
                {
                    serialGPTStatus(card,
                        "KERMIT RX unexpected "+reply.type
                        +" seq="+reply.seq
                        +" while waiting seq="+seq);
                    continue;
                }
                ...
                {
                    serialGPTStatus(card,
                        "KERMIT NAK "+type+" seq="+seq
                        +" attempt="+(attempt+1));
                    continue;
                }

                serialGPTStatus(card,
                    "KERMIT RX unexpected type="+reply.type
                    +" seq="+reply.seq
            ...
            catch(error)
            {
                serialGPTStatus(card,
                    "KERMIT retry "+type
                    +" seq="+seq
                    +" after "+String(error && error.message || error));
    ...
    async function serialGPTKermitWriteFile(card,name,content)
    {
        ...
        try
        {
            // Discard stale packets from a previously aborted transaction.
            // The live early-ACK queue remains active once the first TX occurs.
            k.rx.length = 0;
            k.queue.length = 0;
            k.waiter = null;
            k.lastPacket = null;
            ...
            for(var p=0;p<bytes.length;p+=KRM_TX_DATA)
                await serialGPTKermitSendAndAck(card,seq++,"D",
                    bytes.slice(p,Math.min(bytes.length,p+KRM_TX_DATA)));
