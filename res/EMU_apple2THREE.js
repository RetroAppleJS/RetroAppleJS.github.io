...
//       sceneURL: "res/A2_Screen_beautified_smaller_v3.json",
//       textureFPS: 10,
//       orbitControls: true,
...
(function(global)
{
    ...
    function Apple2VideoTHREE(canvas)
    {
        ...
        this.loadProjectScene = function()
        {
            ...
            if (!cfg.sceneURL || typeof(global.fetch) !== "function")
            {
                console.warn("Apple2VideoTHREE: no sceneURL/fetch available; using fallback scene.");
                return;
            }
            ...
            fetch(cfg.sceneURL)
                ...
                {
                    if (!response.ok)
                        throw new Error("HTTP " + response.status + " while loading " + cfg.sceneURL);
                    return response.json();
                })
                .then(function(json_proj)
                {
                    video3D.installProjectScene(json_proj);
                })
                .catch(function(err)
                {
                    console.warn("Apple2VideoTHREE: could not load project scene; using fallback scene.", err);
                });
        };
