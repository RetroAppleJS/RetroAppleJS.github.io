 ...
 * v6.5 architecture:
     ...
     * ----------------------------------------
     *
     * The S-C Macro Assembler manual calls labels of the form .0 through .99
     * "local labels".  They are not directives, even though their spelling
     * begins with a period.  Each local label is associated with the preceding
     * normal (non-local) label.  The same numeric local label may therefore be
     * reused after another normal label starts a new local-label scope.
     *
     * A reference such as BNE .4 binds to .4 in the scope owned by the most
     * recent preceding normal label.  Forward references inside that scope are
     * legal.  A local label or reference before any normal label is invalid.
     *
     * S-C also restricts a local label's value to no more than 255 bytes above
     * the value of its owning normal label.  This assembler validates that
     * policy when the local label location is registered.
     *
     * ASM_core cannot store the source spelling ".1" directly in the global
     * symbol table because another routine may also define ".1".  During
     * preparsing, every local definition and reference is therefore rewritten
     * to a deterministic private identifier:
     *
     *     .SCLOCAL_<number>_L<anchor-line>_<salt>
     *
     * The leading period keeps the identifier outside the normal user-label
     * namespace.  The anchor line identifies the owning normal-label scope,
     * while the per-assembly salt prevents accidental collisions when rows or
     * symbol tables from separate assembly runs are combined.  sourceSym keeps
     * the original spelling for listings and diagnostics.
...
function ASM(options) 
{
    ...
    this.pragma = {
        ...
        "ASC6":  mkPragma("ASC6",    ["raJS"], parsePragmaData),
        ".ASSERT": mkPragma("ASSERT", ["raJS", "ca65"], parsePragmaAssert),
    ...
    //this.asm_brands = {"ca65":"∞","Merlin":"☾","S-C":"Ş"}
    ...
    this.isSCLocalLabel = function (text) {
        return /^\.(?:[0-9]|[1-9][0-9])$/.test(String(text == null ? "" : text).trim());
    };
    ...
    this.normaliseSCLocalLabels = function (statements) {
        statements = statements || [];
        var salt = ++this.scLocalLabelSalt;
        var anchor = null;
        var localMap = null;
        var defined = null;
        ...
        for (var i = 0; i < statements.length; i++) {
            ...
            if (row.tag[0] === "LBL" && row.sym[0] && !this.isSCLocalLabel(row.sym[0])) {
                ...
                localMap = {};
                defined = {};
                // Keep processing this row: a labelled instruction such as
                // "START BNE .1" opens the scope and may reference it immediately.
            }

            if (!anchor) {
                for (var b = 0; b < row.sym.length; b++) {
                    if (this.isSCLocalLabel(row.sym[b])) {
                        row.err = row.err || "S-C local label " + row.sym[b] + " has no preceding normal label";
                    }
                }
                continue;
            }

            var labelIsLocal = row.tag[0] === "LBL" && this.isSCLocalLabel(row.sym[0]);
            if (labelIsLocal) {
                var labelNumber = String(parseInt(row.sym[0].substring(1), 10));
                if (!localMap[labelNumber])
                    localMap[labelNumber] = this.makeSCLocalLabelID(labelNumber, anchor.line, salt);
                if (defined[labelNumber])
                    row.err = row.err || "duplicate S-C local label ." + labelNumber + " in scope " + anchor.id;
                defined[labelNumber] = true;
                row.sym[0] = localMap[labelNumber];
                row.scLocalLabel = {
                    source: "." + labelNumber,
                    anchor: anchor.id,
                    anchorLine: anchor.line,
                    internal: localMap[labelNumber]
                };
            }

            var firstReference = row.tag[0] === "LBL" ? 1 : 0;
            for (var s = firstReference; s < row.sym.length; s++) {
    ...
    this.registerLabelLocation = function (row, passNo) {
        ...
        if (row.scLocalLabel) {
            ...
            if (typeof anchorValue === "number") {
                var delta = this.pc - anchorValue;
                if (delta < 0 || delta > 255) {
                    row.err = row.err || "S-C local label " + row.scLocalLabel.source
                        + " must be within 0..255 bytes above " + row.scLocalLabel.anchor;
                }
            }
        }
    ...
    this.getNumber = function (text, symtab) {
        ...
        if (hiLo === ">" || hiLo === "<" || hiLo === "/") {
            ...
            var selectedValue = hiLo === "<"
                ? (selected.val & 0xff)
                : ((selected.val >> 8) & 0xff);
            return { val: selectedValue, bytes: 1, fmt: hiLo === "<" ? "LO" : "HI" };
        }
    ...
    /*
     * Convert an assembly expression into simple tokens.  Character literals
     * are converted through appleCharCode(), keeping the Apple II high-bit
     * policy in one place for both plain and compound expressions.
     */
    this.tokenizeExpression = function (text) {
        text = String(text == null ? "" : text);
        var tokens = [];
        var i = 0;
        ...
        function push(type, value, raw) {
            tokens.push({ type: type, value: value, raw: raw });
            if (type === "value" || type === "identifier" || type === "rparen") expectValue = false;
            else if (type === "lparen" || type === "operator") expectValue = true;
        }
        ...
        while (i < text.length) {
            ...
            if (ch === "\"" || ch === "'") {
                ...
                while (i < text.length) {
                    ...
                    if (ch !== "\\") { chars.push(ch); continue; }
                    if (i >= text.length)
                        return { tokens: tokens, err: "unterminated escape in character literal" };
                    ...
                    switch (escaped) {
                        ...
                        case "x":
                            var hexEscape = text.substring(i, i + 2);
                            if (!/^[0-9a-f]{2}$/i.test(hexEscape))
                                return { tokens: tokens, err: "invalid hexadecimal escape in character literal" };
                            chars.push(String.fromCharCode(parseInt(hexEscape, 16)));
                            i += 2;
                            break;
                        default:
                            chars.push(escaped);
                ...
                if (!closed) return { tokens: tokens, err: "unterminated character literal" };
                if (chars.length !== 1)
                    return { tokens: tokens, err: "character literal in expression must contain exactly one character" };

                push("value", asm.appleCharCode(chars[0], quote), quote + chars[0] + quote);
                continue;
            }

            if (ch === "$") {
                var hex = text.substring(i).match(/^\$[0-9a-f]+/i);
                if (hex) {
                    push("value", parseInt(hex[0].substring(1), 16), hex[0]);
                    i += hex[0].length;
                    continue;
                }
            }

            // '%' is a binary-number prefix only where an operand is expected;
            // otherwise it remains the modulo operator (for example 5%2).
            if (ch === "%" && expectValue) {
                var bin = text.substring(i).match(/^%[01]+/);
                if (bin) {
                    push("value", parseInt(bin[0].substring(1), 2), bin[0]);
                    i += bin[0].length;
                    continue;
                }
            }
            ...
            if (/[0-9]/.test(ch)) {
                var dec = text.substring(i).match(/^[0-9]+/)[0];
                push("value", parseInt(dec, 10), dec);
                i += dec.length;
                continue;
            }

            if (/[A-Za-z_.$]/.test(ch)) {
                var id = text.substring(i).match(/^[A-Za-z_.$][A-Za-z0-9_.$]*/)[0];
                push("identifier", id, id);
                i += id.length;
                continue;
            }

            if (ch === "(") { push("lparen", ch, ch); i++; continue; }
            ...
            return { tokens: tokens, err: "invalid expression character: " + ch };
        }
    ...
    this.applyExpressionBinary = function (operator, left, right) {
        switch (operator) {
            case "=":
    ...
    this.getExpression = function (text, symtab) 
    {
        ...
        /*
         * S-C byte selectors apply to the complete expression to their right.
         * Thus /T.NTBL-$100 means high-byte(T.NTBL-$100), not
         * high-byte(T.NTBL)-$100.  '>' remains a high-byte alias and '<' the
         * low-byte alias used by the other supported assembler dialects.
         */
        switch (text.charAt(0)) {
            ...
            case "/":
                ...
                return {
                    val: selectedValue,
                    bytes: 1,
                    fmt: text.charAt(0) === "<" ? "LO" : "HI"
                };

            case "\"":
            case "'":
                // Preserve the existing full-string result used by data pragmas.
                if (text.charAt(text.length - 1) === text.charAt(0)
                    && text.indexOf(text.charAt(0), 1) === text.length - 1) {
                    return this.getNumber(text, symtab);
                }
                break;
         }
 
        var lexed = this.tokenizeExpression(text);
        if (lexed.err) return { val: NaN, err: lexed.err };
        ...
        function peek() {
            return tokens[position] || { type: "eof", value: "", raw: "" };
        }
        ...
        function parsePrimary() {
            ...
            switch (token.type) {
                ...
                case "lparen":
                    var nested = parseBinary(0);
                    if (peek().type !== "rparen") {
                        parseError = parseError || "missing ')'";
                        return nested;
                    }
                    take();
                    return nested;
                ...
                default:
                    parseError = parseError || "expected value, found "
                        + (token.raw || token.value || "end of expression");
                    return 0;
            }
        }
        ...
        function parseUnary() {
            var token = peek();
            if (token.type !== "operator") return parsePrimary();
            ...
            switch (token.value) {
                ...
                case "*": take(); return asm.pc & 0xffff;
        ...
        function parseBinary(minPrecedence) 
        {
            ...
            while (true) 
            {
                ...
                var precedence = asm.expressionPrecedence(token.value);
                if (precedence < minPrecedence) break;

                take();
                var right = parseBinary(precedence + 1);
                try {
                    left = asm.applyExpressionBinary(token.value, left, right);
                } catch (err) {
                    parseError = parseError || err.message;
                }
            }
            return left;
        }        
        ...
        if (!parseError && peek().type !== "eof")
            parseError = "unexpected token: " + (peek().raw || peek().value);

        if (unresolved.length) {
            var uniqueUnresolved = unresolved.filter(function (name, index, arr) {
                return arr.indexOf(name) === index;
            });
            return { val: NaN, err: "unresolved identifier(s): " + uniqueUnresolved.join(", ") };
        }
        if (parseError) return { val: NaN, err: "expression error: " + parseError };

        return {
            val: Number(value),
            bytes: this.operandByteSizeFromValue(Number(value)),
            fmt: "EXP"
        };
    };
    ...
    this.byteCountForDataPragma = function (canonical, operandText, symtab) {
        ...
        switch (canonical) {
            ...
            case "ASC6":
                n = this.stringByteLength(operandText);
                n = Math.ceil((n * 6) / 8);
                return { bytes: n, oby: n };
            case "SC_DA":
                arr = this.splitCSV(operandText);
    ...
    this.compilePragmaRow = function (row, symtab) {
        ...
        switch (ref) {
            ...
            case "ASC6": return this.compileAscii6Data(operands, row);
    ...
    /*
     * ASC6 packs characters from the 64-character ASCII range $20..$5F.
     * The encoded value is simply ASCII-$20, producing 0..63.
     *
     * Four input characters therefore occupy exactly three output bytes:
     *
     *   aaaaaa bbbbbb cccccc dddddd
     *   AAAAAAAA BBBBBBBB CCCCCCCC
     *
     * Bits are emitted MSB first.  A final partial byte is padded with zero
     * bits on the right.  Characters outside $20..$5F are rejected instead
     * of being silently converted; this keeps the packed stream deterministic.
     */
    this.compileAscii6Data = function (operandText, row) {
        ...
        for (var i = 0; i < text.length; i++) {
            ...
            if (code < 0x20 || code > 0x5f) {
                if (row && !row.err) {
                    row.err = "ASC6 character out of range at position "
                        + (i + 1) + ": $" + this.getHexByte(code);
                }
                return [];
            }

            bitBuffer = (bitBuffer << 6) | (code - 0x20);
            bitCount += 6;

            while (bitCount >= 8) {
                bitCount -= 8;
                out.push((bitBuffer >> bitCount) & 0xff);
                bitBuffer &= bitCount ? ((1 << bitCount) - 1) : 0;
            }
        }
    ...
    this.getListingJumpInfo = function (row) {
        ...
        if (!mne && row.tag && row.sym) {
            var mneIndex = this.getTokenIndexByTag(row.tag, "MNE");
            if (mneIndex >= 0) mne = String(row.sym[mneIndex] || "").toUpperCase();
        }
    ...
    this.applyListingLineColumn = function (rows) {
        ...
        for (var i = 0; i < rows.length; i++) {
            var row = rows[i] || {};
            if (row.bytes && row.bytes.length && typeof row.pc === "number")
    ...
    this.getListingColumnWidth = function (columns, key) {
        columns = columns || this.listingColumns || this.defaultListingColumns || {};
    ...
    this.truncateListingLabel = function (label, columns) {
        /*
         * Label display width must follow the active listing column map.
         * The old fixed listingLabelLen default of 8 made wider user settings
         * such as {lbl:21,ins:35} ineffective.
         */
        return this.cropListingField(label, this.getListingColumnWidth(columns, "lbl"));
    };
    ...
    this.getListingParts = function (row, bytes, columns) {
        row = row || {};
        bytes = bytes || [];
        var tag = row.tag || [];
        var sym = row.sym || [];
        ...
        if (lblIndex >= 0) parts.lbl = this.truncateListingLabel(sym[lblIndex], columns);
        if (mneIndex >= 0) {
    ...
    this.formatListingLine = function (row, bytes, columnsOverride) 
    {
        ...
        var line = "";
        /*
         * Keep the legacy raw behaviour for comment-only/source-only style
         * rows, but not for lin-only rows.  A lin-only row is usually a blank
    ...
    /*
     * ca65-style compile-time assertion:
     *
     *   .ASSERT expression, error,   "message"
     *   .ASSERT expression, warning, "message"
     *
     * Assertions are evaluated only on pass 2.  This is intentional: layout
     * passes may still contain unresolved forward labels, while pass 2 sees
     * the final converged symbol table.  ASSERT never changes the PC.
     */
    function parsePragmaAssert(arg) {
        var asm = arg.asm;
        var row = arg.row;
        var passNo = arg.passNo;
        var pgmIndex = asm.getTokenIndexByTag(row.tag, "PGM");
        var operands = row.sym.slice(pgmIndex + 1).join(" ");
        var fields = asm.splitCSV(operands);
        row.asm = this.asm ? this.asm.slice() : ["raJS", "ca65"];

        if (passNo !== 2) return row;

        if (!fields.length) {
            row.err = row.err || ".ASSERT requires an expression";
            return row;
        }

        var expression = fields[0];
        var severity = String(fields[1] || "error").trim().toLowerCase();
        var message = fields.length > 2
            ? asm.decodeSCDelimitedText(fields.slice(2).join(","))
            : "assertion failed: " + expression;

        if (severity !== "error" && severity !== "warning") {
            row.err = row.err || ".ASSERT severity must be error or warning";
            return row;
        }

        var result = asm.getExpression(expression, asm.symtab);
        if (result.err || typeof result.val !== "number" || isNaN(result.val)) {
            row.err = row.err || ".ASSERT " + (result.err || "invalid expression");
            return row;
        }

        if (!result.val) {
            if (severity === "warning") row.warn = row.warn || message;
            else row.err = row.err || message;
        }
        return row;
    }
