...
function EMU_mountDiskImage(arr_buffer,slotN,deviceID,filepath,unit)
{
    ...
    // Let Disk II determine whether a 140K sector image is DOS-order
    // (.dsk) or ProDOS-order (.po) and perform the appropriate conversion.
    if(diskBytes.length===143360)
    {
        disk2.setDiskData(diskBytes,deviceID,filepath);
        return true;
    }
 ...
 * uses an inline onchange="EMU_loadRAM64KFile(this)" handler.
 */
function EMU_loadRAM64KFile(input)
{
    ...
    function clearInput()
    {
        // Allow the same file to be selected again after either success or
        // failure; otherwise many browsers do not fire change a second time.
        try { input.value = ""; } catch(e) {}
    }

    if(file.size != 0x10000)
    {
        alert("64K RAM image must contain exactly 65536 bytes; selected file contains " + file.size + " bytes.");
        clearInput();
        return;
    }
    ...
    reader.onload = function(ev)
    {
        try
        {
            if(typeof(apple2plus)!="object" || !apple2plus)
                throw new Error("Apple II emulator is not initialized.");

            var hw = apple2plus.hwObj();
            if(!hw || typeof(hw.load_ram64k)!="function")
                throw new Error("This emulator build does not provide load_ram64k().");

            var bytes = new Uint8Array(ev.target.result);
            if(bytes.length != 0x10000)
                throw new Error("64K RAM image must contain exactly 65536 bytes.");
            ...
            console.log(
                ...
                "-$" + oCOM.getHexWord(result.to) +
                "; ignored $" + oCOM.getHexWord(result.ignoredFrom) +
                "-$" + oCOM.getHexWord(result.ignoredTo)
            );
        }
        catch(err)
        {
            console.error("RAM64K import failed",err);
            alert("RAM64K import failed: " + (err && err.message ? err.message : err));
        }
        finally
        {
            clearInput();
    ...
    reader.onerror = function()
    {
        ...
        console.error("RAM64K import failed: " + msg);
        alert("RAM64K import failed: " + msg);
        clearInput();
    };
...
function EMU_init()
{
    ...
    for(var uri in oCOM.URL.uri)
    {
        switch(uri)
        {
            ...
            break;
            case "CAT":
                _o.EMU_disk_cat = String(oCOM.URL.uri[uri] || "").trim().toLowerCase() == "true";
            break;
            case "D1":
                    ...
                    function()
                    {
                        ...
                        else
                        {
                            ...
                            try{
                                ...
                                //oCOM.POPUP.html("async call succeeded, boot="+ oCOM.URL.uri["boot"]);
                            }
                            catch({ name, message })
                            {
                                //oCOM.POPUP.html("restart 1.0 failed: "+name+" "+message);
...
*/
    ...
    */
    ...
    // STEP TRACE owns its instruction-row viewport.  EMUI no longer constructs
    // the legacy byte-coordinate TextScroll for the debugger.
    this.cpuDbg = function(id)
    {
        var dbg = oEMU.component.CPU.Apple2Debug;
        var popup = document.getElementById("cpuDbg_popup");
        if(!dbg || !popup) return;

        oCOM.POPUP.toggle("cpuDbg_popup");
        if(popup.hidden===false)
        {
            if(typeof(dbg.open)=="function") dbg.open();
        }
        else if(typeof(dbg.close)=="function")
            dbg.close();
    }
