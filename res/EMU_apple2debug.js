...
function Apple2Debug()
{
    ...
    function conditionValue(node,state)
    {
        ...
        {
            var hw = liveHW();
            if(!hw || typeof(hw.safe_read)!="function") throw new Error("Mapped memory is unavailable");
            var addr = conditionValue(node.a,state) & 0xffff;
            var lo = Number(hw.safe_read(addr));
            if(!Number.isFinite(lo)) throw new Error("Cannot read $"+oCOM.getHexWord(addr));
            lo &= 0xff;
            if(node.w===8) return lo;
            var hi = Number(hw.safe_read((addr+1)&0xffff));
            if(!Number.isFinite(hi)) throw new Error("Cannot read $"+oCOM.getHexWord((addr+1)&0xffff));
            return lo | ((hi & 0xff)<<8);
        }
