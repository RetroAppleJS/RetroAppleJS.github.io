...
function Apple2Debug()
{
    ...
    var conditionHelp = "Conditional breakpoint. Registers/counters: A X Y SP P PC INS. INS is the 48-bit completed-instruction count shown in NAV. Flags: N V B D I Z C. Memory: M[$addr] or MEM[$addr], M16[$addr] or MEM16[$addr]. Operators: == != < <= > >= & | ^ && || ! and parentheses.";
    ...
    function parseBreakpointCondition(text)
    {
        ...
        var pos = 0;
        var registers = {A:1,X:1,Y:1,SP:1,P:1,PC:1,INS:1};
    ...
    function conditionValue(node,state)
    {
        ...
        if(node.t==="reg")
        {
            var key = node.v==="INS" ? "ic" : node.v.toLowerCase();
            if(!state || state[key]===undefined) throw new Error("Register/counter "+node.v+" is unavailable");
    ...
    {
        // Armed/disarmed state is already conveyed by the Arm/Disarm button.
        // A successful hit is shown by the parking icon; textual diagnostics
        // are rendered in the BREAK IF status slot beside the editor.
    ...
    function updateNavigationStatus(pc)
    {
        ...
        if(el)
        {
            var navText = "PC $"+oCOM.getHexWord(pc)
                +"  INS $"+instructionCounterText();
            if(el.value!==navText) el.value = navText;

            var title = followPC
                ? "Listing tracks the live program counter; PC/INS text is read-only and copyable"
                : "Manual instruction-row view; enable Track PC to resume tracking; PC/INS text is read-only and copyable";
            if(conditionalBreakpoint.condition)
                title += "; breakpoint condition: "+conditionalBreakpoint.condition;
            el.title = title;
        }

        // Keep transient run state and breakpoint diagnostics next to BREAK IF,
        // where they relate to the condition editor rather than to PC/INS.
        var status = document.getElementById("cpuDbg_breakStatus");
        if(status)
        {
            var statusText = (boundaryActionText()+breakpointText()).replace(/^\s+|\s+$/g,"");
            status.textContent = statusText;
            status.style.display = statusText ? "" : "none";
            status.title = statusText ? "Debugger run/breakpoint status: "+statusText : "";
        }
    }
    ...
    /*
     * Fixed IPS is paced against an absolute wall-clock deadline.
     *
     * Do not schedule "delay ms after the previous batch": doing so adds CPU,
     * I/O and DOM-rendering time to every interval and causes cumulative drift.
     *
     * The first batch is due after one complete interval. Thus, measured from
     * pressing Run:
     *
     *   1 IPS  -> 1 completed instruction after ~1 s
     *  10 IPS  -> 10 completed instructions after ~1 s
     * 100 IPS  -> 5 instructions every ~50 ms
     * 1000 IPS -> 50 instructions every ~50 ms
     */
    function scheduleFirstFixedRun(cfg)
    {
        fixedNextDueMs = fixedClockMs()+cfg.delay;
        scheduleFixedRun(cfg.delay);
    }

    function scheduleNextFixedRun(cfg)
    {
        var now = fixedClockMs();
        fixedNextDueMs += cfg.delay;

        // Browser suspension/background throttling can make a deadline very old.
        // Do not replay an arbitrarily large backlog in a burst.
        if(fixedNextDueMs < now-Math.max(1000,cfg.delay*4))
            fixedNextDueMs = now+cfg.delay;

        scheduleFixedRun(fixedNextDueMs-now);
    }
    ...
    function startExecution()
    {
        ...
        else
        {
            pauseSystem();
            fixedRunning = true;
            scheduleFirstFixedRun(runSpeedConfig(runMode));
        }
        syncRunIcon();
    }
            ...
            +"<div class=marginless style='border:0;font-family:Arial,sans-serif;font-size:9px;line-height:18px'>"
                    ...
                    +"<div style='display:flex;align-items:center;gap:3px;white-space:nowrap'>"
                        ...
                        +"<button type='button' aria-label='Next instruction; hold for page down' title='Next instruction; hold 1/2s for page down' "
                            ...
                            +"ontouchstart='return oEMU.component.CPU.Apple2Debug.navButtonDown(this,1)' ontouchend='return oEMU.component.CPU.Apple2Debug.navButtonUp(this)' ontouchcancel='return oEMU.component.CPU.Apple2Debug.navButtonCancel(this)' "
                            +"style='border:0;background:transparent;-webkit-appearance:none;appearance:none;padding:0 1px;margin:0;line-height:1;font-size:11px;cursor:pointer'>↓</button>"
                        +"<input id='cpuDbg_navStatus' type='text' readonly value='PC $0000  INS $000000000000' aria-label='Live PC and instruction counter' title='Live PC and instruction counter; read-only and copyable' style='width:146px;height:16px;padding:0 2px;box-sizing:border-box;border:1px solid #bbb;background:#fff;font-family:"+listingFontFamily+";font-size:9px;cursor:text'>"
    ...
    this.cycle = function(obj)
    {
        ...
        if(followPC)
        {
            if(changed || (obj && obj.force))
                renderListing(pc,!!(obj && obj.force));
            else
                // PC-stable code (for example JMP $xxxx to itself) still
                // completes instructions. Keep NAV/INS live without repainting
                // the listing or changing the displayed PC.
                updateNavigationStatus(pc);
        }
        else
            renderListing(pc,!!(obj && obj.force));
