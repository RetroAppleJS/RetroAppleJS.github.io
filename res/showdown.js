...
showdown.helper.escapeHtml = function (text) 
{
  const escapeMap = {
    '&': '&amp;',
    '<': '&lt;',
...
showdown.subParser('makeMarkdown.txt', function (node) {
  ...
  txt = showdown.helper.escapeHtml(txt)        // ", <, > and & should replace escaped html entities
        ...
        .replace(/^(\s*)([-=]{3,})(\s*)$/, '$1\\$2$3')   // horizontal rules
        .replace(/^( {0,3}\d+)\./gm, '$1\\.')            // dot, because of ordered lists, only troublesome at the beginning of a line when preceded by an integer
        .replace(/^( {0,3})([+-])/gm, '$1\\$2')          // +, * and -, at the beginning of a line becomes a list, so we need to escape them also (asterisk was already escaped)
        .replace(/]([\s]*)\(/g, '\\]$1\\(')              // images and links, ] followed by ( is problematic, so we escape it
        .replace(/^ {0,3}\[([\S \t]*?)]:/gm, '\\[$1]:'); // reference URIs must also be escaped
