...
_DOCS["DEBUGGER"] = "<h1 id=\"retroapplejs-debugger-—-user-guide\">RetroAppleJS Debugger — User Guide</h1><hr><hr>"
...
+"<hr />"
+"<h2 id=\"1-quick-start-debug-samples\">1. Quick Start: Debug <code>sample.S</code></h2><hr>"
...
+"</ul>"
+"<hr />"
+"<h2 id=\"2-what-the-assembler-sends-to-the-debugger\">2. What the Assembler Sends to the Debugger</h2><hr>"
...
+"<pre><code class=\"text language-text\">┌────────────────────┐<br>"
+"│  sample.S          │<br>"
+"│  6502 source code  │<br>"
+"└─────────┬──────────┘<br>"
+"          │ Assemble<br>"
...
+"<hr />"
+"<h2 id=\"3-byte-code--symbols--readable-debugging\">3. Byte Code + Symbols = Readable Debugging</h2><hr>"
+"<p>The CPU only executes bytes. Symbols do not change execution. Symbols make execution understandable for humans.</p>"
+"<pre><code class=\"text language-text\">            CPU EXECUTES                         DEBUGGER EXPLAINS<br>"
+"<br>"
+"┌─────────────────────────────┐       ┌─────────────────────────────┐<br>"
...
+"<hr />"
+"<h2 id=\"4-the-debugger-data-sources\">4. The Debugger Data Sources</h2><hr>"
...
+"</table>"
+"<hr />"
+"<h2 id=\"5-the-debugger-screen\">5. The Debugger Screen</h2><hr>"
...
+"└──────────────────────┴──────────────────────┴──────────────────────┘</code></pre>"
+"<hr />"
+"<h2 id=\"6-cpu-watch\">6. CPU Watch</h2><hr>"
...
+"<hr />"
+"<h2 id=\"7-cpu-trace\">7. CPU Trace</h2><hr>"
...
+"<pre><code class=\"text language-text\">$F800  4A        PLOT     LSR A<br>"
+"$F801  08                 PHP<br>"
...
+"<hr />"
+"<h2 id=\"8-listing-caret-and-origin\">8. Listing Caret and Origin</h2><hr>"
...
+"<p>For example:</p>"
+"<pre><code class=\"text language-text\">ORG $6000</code></pre>"
...
+"<hr />"
+"<h2 id=\"9-stepping-through-code\">9. Stepping Through Code</h2><hr>"
...
+"<pre><code class=\"text language-text\">Step → read trace → inspect register changes → compare with source → step again</code></pre>"
...
+"<hr />"
+"<h2 id=\"10-debugging-with-memcap\">10. Debugging with MemCap</h2><hr>"
...
+"<h3 id=\"memcap-workflow\">MemCap workflow</h3>"
+"<ol>"
...
+"┌──────────────────────┐<br>"
+"│ Memory capture       │<br>"
+"│ copied as text/data  │<br>"
+"└──────────┬───────────┘<br>"
+"           │ paste into Debugger Tools<br>"
+"           ▼<br>"
+"┌──────────────────────┐       ┌──────────────────────────────┐<br>"
+"│ Debugger memory      │◀──────│ Optional symbols              │<br>"
+"│ reconstructed image  │       │ e.g. ROM labels/comments      │<br>"
+"└──────────┬───────────┘       └──────────────────────────────┘<br>"
+"           │<br>"
+"           ▼<br>"
+"┌──────────────────────┐<br>"
+"│ Step through memory  │<br>"
+"│ with readable trace  │<br>"
+"└──────────────────────┘</code></pre>"
+"<h3 id=\"example-symbol-file\">Example symbol file</h3>"
+"<p>For the Apple II Monitor ROM, a matching symbol file can be loaded from:</p>"
+"<pre><code class=\"text language-text\">/asm/ROMS/APPLE/APPLE-2 MONITOR ROM.symbols.json</code></pre>"
+"<p>A memory capture gives the debugger the bytes. The symbol file gives those bytes names and comments.</p>"
+"<hr />"
+"<h2 id=\"11-working-with-symbol-files\">11. Working with Symbol Files</h2><hr>"
...
+"<p>Example idea:</p>"
+"<pre><code class=\"text language-text\">Without symbols:<br>"
...
+"<hr />"
+"<h2 id=\"12-debugger-sessions\">12. Debugger Sessions</h2><hr>"
...
+"<hr />"
+"<h2 id=\"13-memory-notes\">13. Memory Notes</h2><hr>"
+"<p>The Apple II uses a 16-bit address space, so the debugger works with a 64K memory range:</p>"
+"<pre><code class=\"text language-text\">$0000 ─────────────────────────────────────────────── $FFFF</code></pre>"
...
+"<hr />"
+"<h2 id=\"14-troubleshooting\">14. Troubleshooting</h2><hr>"
+"<h3 id=\"the-debugger-opens-but-stepping-starts-at-the-wrong-address\">The debugger opens, but stepping starts at the wrong address</h3>"
+"<p>Check:</p>"
+"<ul>"
...
+"</ul>"
+"<h3 id=\"the-trace-shows-raw-addresses-instead-of-labels\">The trace shows raw addresses instead of labels</h3>"
+"<p>Load a matching symbol file, or use the Assembler handoff so the assembler can provide its symbol table automatically.</p>"
+"<h3 id=\"the-trace-does-not-match-the-source\">The trace does not match the source</h3>"
+"<p>Check that:</p>"
+"<ul>"
...
+"</ul>"
+"<h3 id=\"rom-symbols-look-wrong\">ROM symbols look wrong</h3>"
+"<p>Make sure the symbol file belongs to the exact ROM image or memory capture you are debugging.</p>"
+"<h3 id=\"a-memory-capture-does-not-step-correctly\">A memory capture does not step correctly</h3>"
+"<p>Check that:</p>"
+"<ul>"
...
+"</ul>"
+"<hr />"
+"<h2 id=\"15-practical-first-exercise\">15. Practical First Exercise</h2><hr>"
...
+"<pre><code class=\"text language-text\">Source line  → assembled bytes → memory → CPU execution → trace + register changes</code></pre>"
