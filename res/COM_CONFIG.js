...
,"A3R":{"Model":"Apple III Revised" ,"CPU":"6502B" ,"Speeds":"1.8" ,"Display":"A2_hgr" ,"ROMS":"" ,"KeysROM":"A3_US"}
,"A2eA":{"Model":"Apple IIe RevA" ,"CPU":"6502" ,"Speeds":"1.023" ,"Display":"A2_hgr" ,"ROMS":"" ,"KeysROM":"A2e_US,A2e_UK,A2e_CA,A2e_FR"}
,"A2eB":{"Model":"Apple IIe RevB" ,"CPU":"6502" ,"Speeds":"1.023" ,"Display":"A2_dhgr" ,"ROMS":"" ,"KeysROM":"A2e_US,A2e_UK,A2e_CA,A2e_FR"}
,"A2c":{"Model":"Apple IIc" ,"CPU":"65C02" ,"Speeds":"1.023" ,"Display":"A2_dhgr" ,"ROMS":"" ,"KeysROM":""}
,"A3P":{"Model":"Apple III Plus" ,"CPU":"6502B" ,"Speeds":"1.8" ,"Display":"A3_dhgr" ,"ROMS":"" ,"KeysROM":""}
,"A2eE":{"Model":"Apple IIe Enhanced" ,"CPU":"65C02" ,"Speeds":"1.023" ,"Display":"A2_dhgr" ,"ROMS":"" ,"KeysROM":""}
,"A2GS":{"Model":"Apple IIGS" ,"CPU":"65C816" ,"Speeds":"2.8" ,"Display":"AGS" ,"ROMS":"" ,"KeysROM":""}
,"A2cM":{"Model":"Apple IIc MemoryExp" ,"CPU":"65C02" ,"Speeds":"1.023" ,"Display":"A2_dhgr" ,"ROMS":"" ,"KeysROM":""}
,"A2G3":{"Model":"Apple IIGS ROM3" ,"CPU":"65C816" ,"Speeds":"2.8" ,"Display":"AGS" ,"ROMS":"" ,"KeysROM":""}
,"A2eP":{"Model":"Apple IIe Platinum" ,"CPU":"65C02" ,"Speeds":"1.023" ,"Display":"A2_dhgr" ,"ROMS":"" ,"KeysROM":""}
,"A2eC":{"Model":"Apple IIe Card" ,"CPU":"65C02" ,"Speeds":"1.023,1.9" ,"Display":"A2_dhgr" ,"ROMS":"" ,"KeysROM":""}
}
...
+"<p>While the end-user experience with this emulator hardly differs from the real hardware, there are some essential deviations, mostly to accomodate contemporary user expecations, like copy-paste.  There is no point in creating an emulator that is completely offline like it was mostly the case in it's pre-internet era.  This user manual is only discussing the usability deviations of the emulator compared to the bare metal.</p>"
+"<p>For the last one, there is a huge body of <br>"
+"<a href=\"https://mirrors.apple2.org.za/Apple%20II%20Documentation%20Project/Books/\">documentation</a> available, and for more specific details on the global architecture of this very emulator, please check out the following document: <a href=\"EMULATOR_ARCHITECTURE.md\">EMULATOR_ARCHITECTURE.md</a></p>"
...
+"<p><br></p>"
+"<p><strong>Screen features</strong></p>"
+"<p><img src=\"https://github.com/RetroAppleJS/RetroAppleJS.github.io/raw/main/res/appleII_title.png?raw=true\" width=40% align=\"left\"/> </p>"
+"<p>Steve Wozniak's chip-saving design requires quite some technical knowledge to accurately produce video emulation. This involves mapping memory locations to physical coordinates on the screen, reading fonts from character ROM in text mode, working with LORES, HIRES, and mixed modes.  You need to understand artifact color conventions, and the inner workings of the video scanner, which is a DMA device that uses timing to drive video data out of RAM and sends them to the video generator. In screen emulation technology, there are two major solution options: one based on the CPU of the host and another based on GPU.</p>"
+"<p>The video scanner operates similarly to a television scan. Due to different TV standards between Europe and America, NTSC models had a master clock tuned at 14.31818Mhz and PAL around 14.238Mhz, which was then exposed to the CPU after division by a factor of 14, resulting in approximately 1MHz. </p>"
+"<p>The <em>horizontal scanning</em> counter consists of 65 states, 1 state lasting approximately 1µs, with 40 states allocated for display and 25 states called 'horizontal blank'. So far accurately documented, we can still estimate that the beam journey travels across the left (blank) border traversing 5 states, then utilizing 40 states (or columns) for reading video memory and display at a rate of one CPU cycle per TEXT/LORES column or 7 HIRES pixels. The display area is followed by another 5 blank states across the right border to reach the right end, after which the remaining 15 blank states are left to retrace or clear the beam to the left end, where the following line starts. </p>"
+"<p>The <em>vertical scanning</em> counter in NTSC/PAL configuration consist respectively of 262/312 states.  Only 192 states are allocated for display, while again respectively in NTSC/PAL configuration the remaining 70/120 states are reserved for vertical blanking including the top margin, bottom margin and retrace back to the top of the screen.  Again so far accurately documented, we can only roughly estimate that the beam takes 24/49 states to travel across the top (blank) margin, then utilizing 192 states displaying stuff, reaching to the bottom margin taking another 24/49 states, and when finally engaging the beam into the retrace maneuver to the top, taking the remaining 22 states. </p>"
+"<p>*So, why do we need to know this to build an emulator?</p>"
+"<p>All Apple IIs were designed with a data bus driver that alternated CPU and VIDEO to access main memory.  These bus drivers came with adapted transistor circuitry to cope with electric field effects (e.g. capacitance) typically found on larger motherboards.  These effects, however unsupported by Apple, enabled a few helpful hacks that are fun to emulate.  One of these hacks is the <em>vapour lock</em>, allowing the detection of the video beam position without additional hardware or modification, allowing flicker-free gameplay and mixed-mode graphic effects once thought unimaginable.</p>"
+"<pre><code><div style=width:800px><br>"
+"<br>"
+"HORIZONTAL SCANNING                            65 bus cyles (~µs) = traversing 1 horizonal line<br>"
+"                                 │◄───────────────────────────────────────────────────────────►│  <br>"
+"                                 │ 5µs :             40µs               : 5µs :      15µs      │ <br>"
+"                                 │◄───►:◄──────────────────────────────►:◄───►:◄──────────────►│<br>"
+"                                 │     ┌────────────────────────────────┐     :                │<br>"
+"PAL & NTSC display               │left │           display              │right:     beam       │<br>"
+"scan line                       ───────┘            area                └───────────────────────<br>"
+"                                 margin:                                :margin    retrace<br>"
+"                                       ┌┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┬┐<br>"
+"                                       ││││││││││││││││││││││││││││││││││  <br>"
+"Direct Memory Access to RAM     ───────┘      latch-in display data     └─────────────────────────<br>"
+"                                         (40 bytes = TEXT/LORES columns)<br>"
+"                                  ┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐┌┐       ┐<br>"
+"Apple II bus at 1MHz            ──┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└┘└...    ├>  SYSTEM<br>"
+"                                                                                               ┘   CLOCK<br>"
+"<br>"
+"<br>"
+"VERTICAL  SCANNING                            17030 bus cycles (~µs) = traversing 1 screen height<br>"
+"                                 │◄───────────────────────────────────────────────────────────►│  <br>"
+"                                 │1560µs:          12480µs               :1560µs:      1430µs  │ <br>"
+"                                 │◄────►:◄──────────────────────────────►:◄────►:◄────────────►│<br>"
+"                                 │      ┌────────────────────────────────┐      :              │<br>"
+"NTSC display                     │ top  │           display              │bottom:     beam     │<br>"
+"scan line                       ────────┘            area                └───────────────────────<br>"
+"                                 margin                                   margin    retrace<br>"
+"<br>"
+"</div></code></pre>"
