...
+"<li>Assemble the default 'sample.S' source by clicking the blinking <strong>play / assemble</strong> button.</li>"
...
+"<hr />"
+"<h2 id=\"1-quick-start-debug-samples\">1. Quick Start: Debug 'sample.S'</h2><hr>"
...
+"<li>Leave the default 'sample.S' source in place.</li>"
...
+"<li>A debugger session named 'sample.DEB' is prepared.</li>"
...
+"│  • symbol table    │<br>"
+"└─────────┬──────────┘<br>"
+"          │ Send byte code & symbols to debugger<br>"
+"          ▼<br>"
+"┌────────────────────┐<br>"
+"│  sample.DEB        │<br>"
+"│  debugger session  │<br>"
...
+"└────────────────────┘<br>"
+"'''</p>"
+"<p>The '.DEB' session is a practical debugger container. It holds the byte code, the symbol information supplied by the assembler, and the debugger settings as they are configured at that moment.</p>"
+"<hr />"
...
+"<p>'''text<br>"
+"            CPU EXECUTES                         DEBUGGER EXPLAINS</p>"
+"<p>┌─────────────────────────────┐       ┌─────────────────────────────┐<br>"
...
+"<p>The 'SR' register contains eight flags, shown from bit 7 to bit 0:</p>"
+"<table>"
+"<thead>"
+"<tr>"
+"<th id=\"flag\">Flag</th>"
...
+"└───────┘└────┘</p>"
+"<p>N:0 V:0 -:1 B:0 D:0 I:1 Z:0 C:0<br>"
+"'''</p>"
...
+"<p>'''text<br>"
+"$F800  4A        PLOT     LSR A<br>"
+"$F801  08                 PHP<br>"
+"$F802  20 70 F8           JSR GBASCALC<br>"
+"$F805  28                 PLP<br>"
+"'''</p>"
+"<p>When symbols are available, labels such as 'PLOT' or 'GBASCALC' can appear instead of anonymous addresses.</p>"
+"<hr />"
...
+"ORG $6000<br>"
+"'''</p>"
+"<p>means that the first assembled byte belongs at '$6000'.</p>"
...
+"</ol>"
+"<p>If the caret or trace starts at the wrong place, first check that the assembled source has the expected 'ORG' value.</p>"
+"<hr />"
...
+"<li>and flags such as 'Z', 'N', or 'C'.</li>"
+"</ul>"
+"<p>A typical debugging rhythm is:</p>"
+"<p>'''text<br>"
+"Step → read trace → inspect register changes → compare with source → step again<br>"
+"'''</p>"
...
+"<p>'''text<br>"
+"/asm/ROMS/APPLE/APPLE-2 MONITOR ROM.symbols.json<br>"
+"'''</p>"
...
+"<p>A debugger session, such as 'sample.DEB', collects the state needed to continue or reproduce a debugging setup.</p>"
...
+"</ul>"
+"<p>The Assembler handoff prepares 'sample.DEB' automatically when you send byte code to the debugger.</p>"
+"<hr />"
...
+"<ol>"
+"<li>Assemble 'sample.S'.</li>"
+"<li>Send byte code to debugger.</li>"
+"<li>Click <strong>Step one instruction forward</strong>.</li>"
+"<li>Watch 'PC' advance.</li>"
...
+"<p>'''text<br>"
+"Source line  → assembled bytes → memory → CPU execution → trace + register changes<br>"
+"'''</p>"
