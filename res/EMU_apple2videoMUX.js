...
function Apple2VideoMUX(canvas)
{
    ...
    this.getRenderer = function(spec, forceReset)
    {
        ...
        /*
         * Registration only constructs the renderer.  A registered renderer
         * still needs its own reset() before its first activation.
         */
        var needsInitialisation = forceReset || r._muxInitialised !== true;

        if(needsInitialisation && typeof(r.reset) == "function")        
            r.reset();

        r._muxInitialised = true;
    ...
    this.reset = function()
    {
        ...
        /*
         * A machine reset invalidates renderer-specific setup.  Reset the
         * active renderer below; the others reset lazily when selected.
         */
        for(var d=0;d<this.devices.length;d++)
            this.devices[d]._muxInitialised = false;
