...
function col80card()
{
    ...
    var SlotROM = new Uint8Array([
        ...
        ,0xED,0xC8,0xA9,0x80,0x20,0xF5,0xC9,0x20,0x44,0xC8,0xC9,0x9B,0xF0,0xF1,0xC9,0x8D
    ...
    function readHostROM(addr) 
    {
        const idxAddr = addr-this.action.HostROM.base & 0xFF;
        console.log("col80card","readHostROM(0x"+oCOM.getHexMulti(addr+0xC000,4)+" -> "+idxAddr+")");
        return HostROM[addr & 0xFF];
    };

    function readSlotROM(addr) 
    {
        ...
        if(this.action.HostROM && CIO.ACTION_MAP.Hslot != this.state.slot) 
        {
            ...
            //override all the callbacks in HostROM
            for(var i=pinfo.HostROM.from;i<=pinfo.HostROM.to;i++) 
                CIO.ACTION_MAP.RD[i] = this.action.HostROM.RD.callback;
        } 

        const idxAddr = addr-this.action.SlotROM.base & 0xFF;
        console.log("col80card","readSlotROM(0x"+oCOM.getHexMulti(addr+0xC000,4)+" -> "+idxAddr+")");
        return HostROM[idxAddr];
    };

    function read(addr)
    {
        const idxAddr = addr-this.action.SlotIO.base & 0xFF;
        console.log("col80card","read(0x"+oCOM.getHexMulti(addr+0xC000,4)+" -> "+idxAddr+")");
        return 0x60;        // TODO: THE VIDEX NEEDS TO RETURN HERE SOMETHING ?
    }
