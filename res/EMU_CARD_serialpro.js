...
function SerialProCard()
{
    ...
    /*
     * Start a new application-level serial session from a deterministic RX
     * boundary. A normal Apple II/6551 reset intentionally preserves remote
     * bytes already on the simulated line; changing GPT8/GPT16 encoding must
     * not preserve those bytes because they belong to the previous protocol.
     */
    this.serialLineResetReceiveSession = function()
    {
        aciaSyncNow();

        var info = {
             queued:serialTerminalState.toCard.length
            ,rxFull:!!acia.rxFull
            ,rxBusy:!!acia.rxBusy
            ,rxData:acia.rxData & 0xFF
            ,rxShift:acia.rxShift & 0xFF
        };

        serialTerminalState.toCard.length = 0;
        acia.rxData = 0x00;
        acia.rxFull = false;
        acia.rxErrors = 0x00;
        acia.rxShift = 0x00;
    ...
    this.serialLineReceiveInfo = function()
    {
        return {
             queued:serialTerminalState.toCard.length
            ,rxFull:!!acia.rxFull
