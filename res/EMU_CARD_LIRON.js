...
function AppleLiron()
{
    ...
    function deviceToolArchiveFilename(filename,hardDisk)
    {
    ...
    this.deviceToolLoadFile = function(input,unit)
    {
        ...
        if(!Number.isInteger(expectedBytes) || expectedBytes<=0 || (!gzipHD20 && Number(file.size)!==expectedBytes))
        {
        ...
        reader.onload=function(ev)
        {
            try
            {
                ...
                if(gzipHD20)
                {
                    ...
                    if(typeof pako === "undefined" || !pako || !pako.ungzip)
                    {
    ...
    this.deviceToolDownload = function(unit)
    {
        ...
        if(hardDisk)
        {
            if(typeof pako === "undefined" || !pako || !pako.gzip)
            {
    ...
    this.deviceToolSyncMediaControls = function(unit,options)
    {
        ...
        if(download)
        {
            ...
            download.title=downloadable ? ("Save "+downloadFilename) : (exportable ? "Save disk (no media loaded)" : "Save disk (not implemented yet)");
    ...
    this.deviceToolSlotHTML = function(ctx)
    {
        ...
        for(var i=0;i<devices.length;i++)
        {
            ...
            rows += EMU_deviceMediaRowHTML({
                ...
                ,"downloadTitle":downloadable ? ("Save "+downloadFilename) : (exportable ? "Save disk (no media loaded)" : "Save disk (not implemented yet)")
