...
function PANE()
{
    ...
    this.bindSearch = function(wrapper, target, callback)
    {
        ...
        var findEl = wrapper.querySelector(".pane-find");
        var replEl = wrapper.querySelector(".pane-replace");
        var reEl   = wrapper.querySelector(".pane-regexp");
        var caseEl = wrapper.querySelector(".pane-case");
        if(!findEl) return;

        function flags(global)
        {
            return (global ? "g" : "") + (caseEl && caseEl.checked ? "" : "i");
        }

        function pattern(global)
        {
            var s = findEl.value;
            if(!s) return null;
            if(reEl && reEl.checked) return new RegExp(s, flags(global));
            return new RegExp(s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), flags(global));
        }
        ...
        function find(next)
        {
            ...
            var txt = target.value || target.textContent || "";
            var from = next ? target.selectionEnd || 0 : 0;
            var m = txt.slice(from).match(re);
            if(!m && from > 0) m = txt.match(re), from = 0;
            if(!m) return;

            var pos = from + m.index;
            if(target.setSelectionRange) {
                target.focus();
                target.setSelectionRange(pos, pos + m[0].length);
            }
        }
        ...
        function replace(all)
        {
            ...
            if(target.value !== undefined) target.value = target.value.replace(re, replEl ? replEl.value : "");
            else target.textContent = target.textContent.replace(re, replEl ? replEl.value : "");

            if(typeof callback == "function") callback(target);
        }

        wrapper.addEventListener("click", function(ev) {
            var act = ev.target && ev.target.dataset ? ev.target.dataset.paneAct : "";
            if(act == "find-prev") find(false);
            if(act == "find-next") find(true);
            if(act == "replace") replace(false);
            if(act == "replace-all") replace(true);
        });
