...
function prettyJsonAllman(value, indent) {
  ...
  this.ASCII_GRID = function(cols,rows,fill)
  {
    const COLS = Math.max(1,Number(cols) || 1);
    const ROWS = Math.max(1,Number(rows) || 1);
    const FILL = (fill == null ? " " : String(fill)).charAt(0) || " ";
    const ascii = [];
    ...
    {
      ch = (ch == null ? FILL : String(ch).charAt(0)) || " ";
    ...
    /*
     * Visual-layer adapter for RetroAppleJS.
     * This is the hook AsciiCAD uses to update the grid and build undo data.
     */
    this.applyOpAtCell = function(cell,op)
     {
        if(!cell || cell.r < 0 || cell.r >= ROWS || cell.c < 0 || cell.c >= COLS) return;
      const prev = ascii[cell.r][cell.c];
      const next = (op.type === "place") ? op.ch : FILL;
      if (prev === next) return;
    ...
    {
      if(stroke && stroke.length) this.undoStack.push(stroke.slice());
    }

    this.draw = function()
    {
      /* no-op: RetroAppleJS renders this text grid into a readonly textarea */
    }

    /*
     * AsciiCAD-compatible text placement.
     * Original API: cell(<c>,<r>,<string>)
     */
    ...
    /*
     * AsciiCAD-compatible box.
     * Original API: box(<c0>,<r0>,<c1>,<r1>, {kind:[boxStyle]})
     */
    ...
    this.line = function(arg)
    {
      ...
      for(var i=1;i<path.length;i++)
      {
        ...
        while(c0 != c1)
        {
          this.applyOpAtCell({c:c0,r:r0},{ch:k.h,type:"place"});
          c0 += dc;
        }
        ...
        while(r0 != r1)
        {
          this.applyOpAtCell({c:c0,r:r0},{ch:k.v,type:"place"});
          r0 += dr;
        }

        this.applyOpAtCell({c:c1,r:r1},{ch:(c0 == c1 ? k.v : k.h),type:"place"});
      }
      this.pushStrokeIfNonEmpty(this._currentStroke);
    ...
    {
      return ascii.map(function(row){ return row.join("").replace(/\s+$/,""); }).join("\n");
    };
  };

  this.textGrid = function(cols,rows,fill)
  {
    return new this.ASCII_GRID(cols,rows,fill);
  };
...
function PANE()
{
    ...
    /*
     * Common pane skin owned by PANE().
     * Page files should not define pane/header/body fonts, colors, dark-mode
     * foreground/background, or pane background patterns.  They should only
     * provide layout constraints, content, ids, and callbacks.
     */
    this.dotMatrixImage = 'url("data:image/png;base64,")';

    this.paneCSS = function()
    {
        return ''
        + '.com-pane-host{display:grid;grid-template-columns:repeat(var(--com-pane-visible-count,1),minmax(0,1fr));gap:10px;width:100%;align-items:start;}'
        + '.com-pane-host>.com-pane[hidden]{display:none!important;}'
        + '.com-pane{min-width:0;max-width:100%;display:flex;flex-direction:column;gap:2px;border-radius:10px;overflow:visible;'
            + '--com-pane-light-header-bg:rgba(244,244,244,.88);--com-pane-light-header-fg:#000;--com-pane-light-header-border:rgba(0,0,0,.18);'
            + '--com-pane-light-body-bg:#f4f4f4;--com-pane-light-body-fg:#000;--com-pane-light-body-border:rgba(0,0,0,.12);'
            + '--com-pane-dark-header-bg:#111820;--com-pane-dark-header-fg:#e6edf3;--com-pane-dark-header-border:#30363d;'
            + '--com-pane-dark-body-bg:#000;--com-pane-dark-body-fg:#e6edf3;--com-pane-dark-body-border:#30363d;'
            + '--com-pane-body-font:500 12px/21px courier,monospace;--com-pane-title-font:12px/20px arial,sans-serif;'
            + '--com-pane-pattern-image:' + this.dotMatrixImage + ';--com-pane-pattern-size:585px 42px;}'
        + '.com-pane-title{display:flex;align-items:center;justify-content:space-between;gap:10px;min-height:34px;margin:0 0 2px 0;padding:4px 8px;'
            + 'border-radius:10px;border:1px solid var(--com-pane-light-header-border);background:var(--com-pane-light-header-bg);color:var(--com-pane-light-header-fg);'
            + 'font:var(--com-pane-title-font);box-sizing:border-box;overflow:visible;position:relative;}'
        + '.com-pane-title-left{display:flex;align-items:center;gap:8px;min-width:0;white-space:nowrap;}'
        + '.com-pane-title-right{display:inline-flex;align-items:center;justify-content:flex-end;gap:4px;flex-wrap:nowrap;margin-left:auto;}'
        + '.com-pane-title strong{color:inherit;font-size:13px;font-weight:700;}'
        + '.com-pane-info{color:rgba(0,0,0,.70);font:11px/1.2 arial,sans-serif;white-space:nowrap;}'
        + '.com-pane-action{display:inline-flex;align-items:center;justify-content:center;min-width:20px;height:20px;padding:2px;margin:0;border:0;border-radius:3px;'
            + 'background:rgba(255,255,255,.4);color:inherit;cursor:pointer;font:13px/1 arial,sans-serif;}'
        + '.com-pane-action:hover{background:rgba(255,255,255,.75);}'
        + '.com-pane-body{width:100%;max-width:100%;min-height:var(--com-pane-body-min-height,420px);height:var(--com-pane-body-height,auto);'
            + 'padding:0 30px;border:1px solid var(--com-pane-light-body-border);border-radius:10px;box-sizing:border-box;'
            + 'background-color:var(--com-pane-light-body-bg);color:var(--com-pane-light-body-fg);font:var(--com-pane-body-font);tab-size:4;white-space:pre;'
            + 'overflow-x:auto;overflow-y:var(--com-pane-body-overflow-y,hidden);resize:none;}'
        + '.com-pane-body-pattern{background-image:var(--com-pane-pattern-image);background-size:var(--com-pane-pattern-size);background-attachment:local;background-repeat:repeat;}'
        + '.com-pane-body-plain{background-image:none;}'
        + '.com-pane-body-auto-grow{height:auto;overflow-y:hidden;}'
        + '.com-pane-body-scroll{overflow-y:auto;}'
        + '.com-pane-theme-dark .com-pane-title{border-color:var(--com-pane-dark-header-border);background:var(--com-pane-dark-header-bg);color:var(--com-pane-dark-header-fg);}'
        + '.com-pane-theme-dark .com-pane-info{color:rgba(230,237,243,.72);}'
        + '.com-pane-theme-dark .com-pane-action{background:rgba(255,255,255,.08);color:var(--com-pane-dark-header-fg);}'
        + '.com-pane-theme-dark .com-pane-action:hover{background:rgba(255,255,255,.16);}'
        + '.com-pane-theme-dark .com-pane-body{border-color:var(--com-pane-dark-body-border);background-color:var(--com-pane-dark-body-bg);background-image:var(--com-pane-dark-body-image,none);color:var(--com-pane-dark-body-fg);caret-color:var(--com-pane-dark-body-fg);}'
        + '.com-pane-theme-dark .com-pane-body *{color:var(--com-pane-dark-body-fg);}'
        + '.com-pane-theme-dark .com-pane-highlight-current{background:rgba(255,214,64,.62)!important;color:#000!important;}'
        + '.com-pane-theme-dark .com-pane-highlight-current *{color:#000!important;}';
    };

    this.injectPaneCSS = function()
    {
        if(document.getElementById("com_pane_core_css")) return;
        var style = document.createElement("style");
        style.id = "com_pane_core_css";
        style.textContent = this.paneCSS();
        document.head.appendChild(style);
    };

    this.setTheme = function(root, theme)
    {
        if(typeof root == "string") root = document.getElementById(root);
        if(!root) return false;
        var dark = (theme === true || theme === "dark");
        root.classList.toggle("com-pane-theme-dark", dark);
        root.classList.toggle("com-pane-theme-light", !dark);
        return dark;
    };

    this.toggleTheme = function(root)
    {
        if(typeof root == "string") root = document.getElementById(root);
        if(!root) return false;
        return this.setTheme(root, !root.classList.contains("com-pane-theme-dark"));
    };

    this.syncVisiblePanes = function(host)
    {
        if(typeof host == "string") host = document.getElementById(host);
        if(!host) return 0;
        var panes = Array.prototype.slice.call(host.children).filter(function(el){
            return el.classList && el.classList.contains("com-pane") && !el.hidden && getComputedStyle(el).display !== "none";
        });
        host.style.setProperty("--com-pane-visible-count", Math.max(1, panes.length));
        return panes.length;
    };

    this.actionButtonHTML = function(action)
    {
        action = action || {};
        return '<button type="button" class="com-pane-action ' + this.escapeHTML(action.class || "") + '"'
            + (action.id ? ' id="' + this.escapeHTML(action.id) + '"' : '')
            + (action.act ? ' data-pane-default-act="' + this.escapeHTML(action.act) + '"' : '')
            + (action.onclick ? ' onclick="' + this.escapeHTML(action.onclick) + '"' : '')
            + (action.title ? ' title="' + this.escapeHTML(action.title) + '"' : '')
            + '>' + (action.html || (action.icon ? '<i class="' + this.escapeHTML(action.icon) + '"></i>' : this.escapeHTML(action.text || ""))) + '</button>';
    };

    this.headerHTML = function(header)
    {
        header = header || {};
        var left = header.leftHTML || ('<strong>' + this.escapeHTML(header.title || "") + '</strong>'
            + (header.info ? '<span class="com-pane-info"' + (header.infoId ? ' id="' + this.escapeHTML(header.infoId) + '"' : '') + '>' + this.escapeHTML(header.info) + '</span>' : '')
            + (header.inputHTML || ""));
        var actions = (header.actions || []).map(this.actionButtonHTML.bind(this)).join("");
        var right = header.rightHTML || actions;
        return '<div class="com-pane-title ' + this.escapeHTML(header.class || "") + '">'
            + '<div class="com-pane-title-left">' + left + '</div>'
            + '<div class="com-pane-title-right">' + right + '</div>'
            + '</div>';
    };

    this.bodyClass = function(body)
    {
        body = body || {};
        return this.classHTML(
            "com-pane-body",
            body.pattern === false ? "com-pane-body-plain" : "com-pane-body-pattern",
            body.grow ? "com-pane-body-auto-grow" : "",
            body.scroll ? "com-pane-body-scroll" : "",
            body.class || ""
        );
    };
    ...
    this.getHTML = function(txt, cfg)
     {
        ...
        var sectionClass = this.classHTML(this.rootClass, cfg.class || "");
        var bodyClass = this.bodyClass(body);       

        return ''
            + '<section'
            + (cfg.id ? ' id="' + this.escapeHTML(cfg.id) + '"' : '')
            + ' class="' + this.escapeHTML(sectionClass) + '">'
            + (header.html || this.headerHTML(header))
            + '<' + btag
            + (body.id ? ' id="' + this.escapeHTML(body.id) + '"' : '')
            + ' class="' + this.escapeHTML(bodyClass) + '"'
    ...
    {
        return ''
        + '.com-pane-fr-wrap{position:relative;width:100%;height:100%;min-height:0;overflow:hidden;}'
        ...
        + '.com-pane-fr-overlay{pointer-events:none;color:inherit;background:transparent;}'
        + '.com-pane-fr-wrap>textarea{resize:none;background:transparent!important;color:transparent!important;caret-color:currentColor;}'
    ...
    this.bindFindReplace = function(target, cfg)
    {
        ...
        function openSearch(expanded) {
            ...
            bar.classList.toggle("expanded", expandedState);
