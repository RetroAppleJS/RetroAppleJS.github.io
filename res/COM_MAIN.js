...
function PANE()
{
    ...
    this.findReplaceCSS = function()
    {
        ...
        + '.pane-findreplace{position:fixed;top:8px;left:8px;right:8px;z-index:2000;display:none;'
            + 'grid-template-columns:auto minmax(150px,392px) auto auto auto auto auto;grid-template-rows:24px;'
            + 'align-items:center;gap:4px;padding:2px 4px;border:1px solid #d2d2d2;border-radius:4px;background:#f3f3f3;'
            + 'box-shadow:0 2px 12px rgba(0,0,0,.18);color:#5f5f5f;}'
        ...
        + '.pane-fr-expander,.pane-fr-btn{display:inline-flex;align-items:center;justify-content:center;flex:0 0 20px;'
            ...
            + 'background:transparent;color:#9a9a9a;cursor:pointer;font:14px/1 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;'
            + 'text-align:center;vertical-align:middle;}'
        + '.pane-fr-expander{color:#333;}'
        + '.pane-fr-expander:hover,.pane-fr-expander.active,.pane-fr-btn.active{border-color:#0078d4;background:#e9f4ff;color:#111;}'
        + '.pane-fr-btn:hover{color:#555;background:#ececec;}'
        ...
        + '.pane-fr-field{display:flex;height:20px;border-radius:3px;background:#fff;overflow:hidden;}'
        + '.pane-fr-field input{min-width:0;flex:1;padding:0 2px;border:0;outline:0;font:10px/1 sans-serif;overflow:hidden;color:#5f5f5f;background:#fff;}'
    ...
    this.bindFindReplace = function(target, cfg)
    {
        ...
        if(!bar) {
            document.body.insertAdjacentHTML("beforeend", this.findReplaceHTML(uid, cfg));
