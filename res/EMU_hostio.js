...
function AppleBoard()
{
    ...
    this.deviceList_html = function(model)
    {
        ...
        function deviceIconClass(deviceCode,icon)
        {
            var fallback = {
                 "A2KBD":"fa fa-keyboard"
                ,"A2SPK":"fa fa-volume-up"
            };

            var cls = String(icon || fallback[deviceCode] || "fa fa-microchip").trim();

            // Accept either "fa-keyboard" or "fa fa-keyboard" in device metadata.
            if(cls.indexOf("fa-") < 0)
                cls = "fa-" + cls;

            if(!/(^|\s)fa(s|r|l|d|b)?(\s|$)/.test(cls))
                cls = "fa " + cls;

            /*
             * FontAwesome Free 5 exposes solid icons through .fas.  Keep legacy
             * .fa metadata working, but add .fas so host-device labels render
             * reliably when the class came from device.id.icon.
             */
            if(!/(^|\s)fas(\s|$)/.test(cls))
                cls += " fas";

            if(!/(^|\s)fa-fw(\s|$)/.test(cls))
                cls += " fa-fw";
        ...
        for(var d=0;d<devices.length;d++)
        {
            ...
            var iconClass = deviceIconClass(deviceCode,id.icon);
